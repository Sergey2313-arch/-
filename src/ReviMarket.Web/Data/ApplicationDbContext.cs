...
public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
{
    ...
    protected override void OnModelCreating(ModelBuilder builder)
    {
        ...
        builder.Entity<MarketItem>().HasOne(x => x.Owner).WithMany().HasForeignKey(x => x.OwnerId).OnDelete(DeleteBehavior.SetNull);
        builder.Entity<MarketItem>().HasOne(x => x.AssignedExecutor).WithMany().HasForeignKey(x => x.AssignedExecutorId).OnDelete(DeleteBehavior.SetNull);
        ...
        builder.Entity<OrderCase>().HasOne(x => x.MarketItem).WithMany().HasForeignKey(x => x.MarketItemId).OnDelete(DeleteBehavior.SetNull);
        builder.Entity<OrderCase>().HasOne(x => x.CreatedBy).WithMany().HasForeignKey(x => x.CreatedById).OnDelete(DeleteBehavior.SetNull);
        builder.Entity<OrderCase>().HasOne(x => x.Agent).WithMany().HasForeignKey(x => x.AgentId).OnDelete(DeleteBehavior.SetNull);
        builder.Entity<Wallet>().HasOne(x => x.User).WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Cascade);
        builder.Entity<PaymentTransaction>().HasOne(x => x.User).WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Cascade);
        builder.Entity<PaymentInvoice>().HasOne(x => x.User).WithMany().HasForeignKey(x => x.UserId).OnDelete(DeleteBehavior.Cascade);
        builder.Entity<Deal>().HasOne(x => x.Customer).WithMany().HasForeignKey(x => x.CustomerId).OnDelete(DeleteBehavior.Restrict);
        builder.Entity<Deal>().HasOne(x => x.Executor).WithMany().HasForeignKey(x => x.ExecutorId).OnDelete(DeleteBehavior.Restrict);
